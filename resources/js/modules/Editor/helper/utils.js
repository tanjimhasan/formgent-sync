...
const cleanDraggingHistory = (currentDragFieldRef, spacerInsertedRef, setInserterOverlayActiveField, setDroppedOverlayActiveField) => {
...
const handleDragOver = (event, fields, spacerInsertedRef, updateFormFields) => {
    ...
    if (activeData.fromSidebar) {
        ...
        if (!spacerInsertedRef.current) {
          ...
          updateFormFields(updatedFields.filter((f) => f.type !== "spacer"))
          ...
          updatedFields = arrayMove(updatedFields, spacerIndex, overData.index);
          updateFormFields(updatedFields);
        }
      }
}
...
const handleDragEnd = (
    ...
    let updatedFields = [...fields]
    if(!over){
        cleanDraggingHistory(currentDragFieldRef, spacerInsertedRef, setInserterOverlayActiveField, setDroppedOverlayActiveField);
    ...
    if(nextField){
        const overData = extractEventData(over);
        const spacerIndex = updatedFields.findIndex((field => field.type === 'spacer'));
